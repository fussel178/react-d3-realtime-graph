...
function createNode(width: number, height: number): ExtendedSVGSVGElement {
    ...
    return Object.assign(chart.node(), {
        update(data: DataSample[]) {
            ...
            // build axis renderer
            const xAxis = (g: d3.Selection<SVGGElement, undefined, null, undefined>) => g
                .attr('transform', `translate(0, ${y.range()[0]})`)
                .call(d3.axisBottom(x).tickSizeOuter(0));

            const yAxis = (g: d3.Selection<SVGGElement, undefined, null, undefined>) => g
            ...
            yG.call(yAxis);
            path.attr('transform', `translate(${x(data[0].timeStamp - tickInterval) - 2 * margin})`);
            xG.call(xAxis);
        }
    });
}
